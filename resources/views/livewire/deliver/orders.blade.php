    ...
    <div class="max-w-6xl mx-auto space-y-6 px-4">
        ...
        <div class="bg-neutral-900 rounded-2xl border border-neutral-800 shadow-lg shadow-black/40 overflow-hidden">
            <div class="px-4 py-4 border-b border-neutral-800">
                <p class="text-lg font-semibold text-white">Текущие заказы</p>
                <p class="text-sm text-neutral-400">Карточки с действиями доставки и возврата.</p>
            </div>
            <div class="p-4">
                    ...
                    @forelse ($orders as $order)
                        <div class="bg-neutral-950/60 rounded-2xl border border-neutral-800 p-4 space-y-4">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                <flux:button variant="primary" color="lime" size="sm" wire:confirm
                                    wire:click="markDelivered({{ $order->id }})">
                                    Доставлено
                                </flux:button>
                                <flux:button variant="primary" color="red" size="sm" wire:confirm
                                    wire:click="markReturned({{ $order->id }})">
                                    Возврат
                                </flux:button>
                            </div>
                        </div>
