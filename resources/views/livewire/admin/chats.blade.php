<div>
    ...
    <div class="bg-white p-2 rounded-xl border border-gray-200 space-y-3 mt-5">
        <div class="grid lg:grid-cols-3 gap-2 h-[calc(100vh-200px)]">
            <div class="h-full bg-neutral-100 rounded-lg p-2 space-y-2 {{ $active_chat ? " hidden":"" }} lg:block
                overflow-y-scroll">
            ...
            <div class="h-full {{ $active_chat ? " ":" hidden lg:block" }} bg-neutral-100 rounded-lg col-span-2 p-2
                ...
                <div class="h-full space-y-2 flex flex-col-reverse pb-28 overflow-hidden overflow-y-scroll">
                    ...
                    <div class="w-full grid justify-end">
                        <div class="bg-lime-600 rounded-l-2xl rounded-br-2xl max-w-72 p-2 w-fit min-w-32 text-white">
                            {{ $message->message }}
                        </div>
                        <span class="text-neutral-500 text-xs mt-1">{{ $message->created_at->format('H:i | d.m.Y')
                            }}</span>
                    </div>
                    @else
                    <div class="w-full grid justify-start">
                        <div class="bg-neutral-400 rounded-r-2xl rounded-bl-2xl max-w-72 p-2 w-fit min-w-32 text-white">
                            {{ $message->message }}
                        </div>
                        <span class="text-neutral-500 text-xs text-end mt-1">{{ $message->created_at->format('H:i |
                            d.m.Y')
                            }}</span>
                    </div>
                    @endif
